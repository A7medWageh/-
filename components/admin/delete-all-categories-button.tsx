  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "تم حذف جميع الفئات بنجاح! ✅",
          description: "جميع الفئات تم حذفها نهائياً",
          variant: "success",
        })
        ...
        setErrorMessage(data.error || 'حدث خطأ أثناء حذف الفئات')
        toast({
          title: "فشل في حذف الفئات ❌",
          description: data.error || 'حدث خطأ أثناء حذف الفئات',
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-md">
        {deleteStatus === 'success' ? (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-muted-foreground mt-2">جميع الفئات تم حذفها</p>
          </div>
        ) : (
          <>
            <DialogHeader>
              <DialogTitle className="text-right">حذف جميع الفئات</DialogTitle>
            ...
            <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 flex gap-3 my-4">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  سيتم حذف جميع الفئات بشكل نهائي. لا يمكن استرجاعها.
                </p>
              </div>
            </div>
